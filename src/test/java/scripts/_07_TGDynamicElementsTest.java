...
Box 1, Box 2
     */
    @Test
    public void validateDynamicElementBothBoxes(){
        List<WebElement> elements= driver.findElements(By.cssSelector("p[id*='box_']"));
        String[] boxes= {"Box 1", "Box 2"};
        for (int i = 0; i <elements.size(); i++) {
            Assert.assertTrue(elements.get(i).isDisplayed());
            Assert.assertEquals(elements.get(i).getText(), boxes[i]);
        }
    }
}
